const isNodeOpen = (nodeName) => {
  try {
    const closedNodes = JSON.parse(
      localStorage.getItem('user-closed-nodes') || '[]',
    )
    return !closedNodes.includes(nodeName)
  } catch (error) {
    return true
  }
}

export default {
  proj_presets: [
    'EPSG:4326',
    'EPSG:2294',
    'EPSG:2295',
    'EPSG:2950',
    'EPSG:3005',
    'EPSG:3408',
    'EPSG:3410',
    'EPSG:3571',
    'EPSG:3572',
    'EPSG:3573',
    'EPSG:3574',
    'EPSG:3575',
    'EPSG:3576',
    'EPSG:3857',
    'EPSG:3978',
    'EPSG:3995',
    'EPSG:4267',
    'EPSG:4269',
    'EPSG:26707',
    'EPSG:26708',
    'EPSG:26709',
    'EPSG:26710',
    'EPSG:26711',
    'EPSG:26712',
    'EPSG:26713',
    'EPSG:26714',
    'EPSG:26715',
    'EPSG:26716',
    'EPSG:26717',
    'EPSG:26718',
    'EPSG:26719',
    'EPSG:26720',
    'EPSG:26721',
    'EPSG:26722',
    'EPSG:26907',
    'EPSG:26908',
    'EPSG:26909',
    'EPSG:26910',
    'EPSG:26911',
    'EPSG:26912',
    'EPSG:26913',
    'EPSG:26914',
    'EPSG:26915',
    'EPSG:26916',
    'EPSG:26917',
    'EPSG:26918',
    'EPSG:26919',
    'EPSG:26920',
    'EPSG:26921',
    'EPSG:26922',
    'EPSG:32187',
    'EPSG:32188',
    'EPSG:32198',
    'EPSG:32661',
    'EPSG:42101',
    'EPSG:42102',
    'EPSG:42304',
    'EPSG:102100',
    'EPSG:102185',
    'EPSG:900913',
    'AUTO2:42003',
  ],
  presets: [
    {
      Title_en: 'Alerts / Current Weather',
      Title_fr: 'Alertes / Météo actuelle',
      Name: 'Alerts / Current weather',
      isLeaf: false,
      isOpen: isNodeOpen('Alerts / Current weather'),
      isPreset: true,
      children: [
        {
          Title_en: 'Alerts & Current Conditions',
          Title_fr: 'Alertes & Conditions actuelles',
          Name: 'Alerts Current conditions',
          Img: 'Alerts Current conditions',
          isLeaf: false,
          children: [
            {
              Title: 'MetNotes',
              Name: 'METNOTES',
              isLeaf: true,
              isTemporal: false,
              legendDisplayed: false,
              opacity: 0.75,
            },
            {
              Title: 'Current Weather Alerts',
              Name: 'Current-Alerts',
              isLeaf: true,
              isTemporal: false,
              legendDisplayed: true,
              opacity: 0.75,
            },
            {
              Title: 'Current Conditions',
              Name: 'CURRENT_CONDITIONS',
              isLeaf: true,
              isTemporal: false,
              legendDisplayed: false,
              opacity: 1,
            },
          ],
        },
        {
          Title_en: 'Hurricane Tracks',
          Title_fr: "Trajectoires d'ouragans",
          Name: 'Hurricane tracks',
          Img: 'Hurricane tracks',
          isLeaf: false,
          children: [
            {
              Title: 'Hurricane Response Zone',
              Name: 'HURRICANE_RESPONSE_ZONE',
              isLeaf: true,
              isTemporal: true,
              opacity: 1,
              legendDisplayed: false,
            },
            {
              Title: 'Hurricane Track Forecast Error',
              Name: 'Hurricane-ErrorCone',
              isLeaf: true,
              isTemporal: true,
              opacity: 1,
              legendDisplayed: false,
            },
            {
              Title: 'Hurricane Wind Forecast Wind Radii',
              Name: 'Hurricane-WindRadii',
              isLeaf: true,
              isTemporal: true,
              opacity: 1,
              legendDisplayed: false,
            },
            {
              Title: 'Hurricane Line Segments',
              Name: 'Hurricane-Tracks',
              isLeaf: true,
              isTemporal: true,
              opacity: 1,
              legendDisplayed: false,
            },
            {
              Title: 'Hurricane Forecast Location',
              Name: 'Hurricane-Cyclones',
              isLeaf: true,
              isTemporal: true,
              opacity: 1,
              legendDisplayed: true,
            },
          ],
        },
        {
          Title_en: 'Thunderstorm Outlook - Atlantic',
          Title_fr: 'Potentiel orageux - Atlantique',
          Name: 'TSO Atlantic',
          Img: 'TSO Atlantic',
          isLeaf: false,
          children: [
            {
              Title: 'Thunderstorm Outlook - Atlantic',
              Name: 'Thunderstorm-Outlook_Atlantic',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: true,
              opacity: 0.75,
            },
          ],
        },
        {
          Title_en: 'Thunderstorm Outlook - ON-QC',
          Title_fr: 'Potentiel orageux - ON-QC',
          Name: 'TSO ON-QC',
          Img: 'TSO ON-QC',
          isLeaf: false,
          children: [
            {
              Title: 'Thunderstorm Outlook - QC',
              Name: 'Thunderstorm-Outlook_QC',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
            {
              Title: 'Thunderstorm Outlook - ON',
              Name: 'Thunderstorm-Outlook_ON',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: true,
              opacity: 0.75,
            },
          ],
        },
        {
          Title_en: 'Thunderstorm Outlook - Prairies-NWT',
          Title_fr: 'Potentiel orageux - Prairies-NWT',
          Name: 'TSO Prairies-NWT',
          Img: 'TSO Prairies-NWT',
          isLeaf: false,
          children: [
            {
              Title: 'Thunderstorm Outlook - NWT',
              Name: 'Thunderstorm-Outlook_NWT',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
            {
              Title: 'Thunderstorm Outlook - Prairies',
              Name: 'Thunderstorm-Outlook_Prairies',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: true,
              opacity: 0.75,
            },
          ],
        },
        {
          Title_en: 'Thunderstorm Outlook - BC-YT',
          Title_fr: 'Potentiel orageux - BC-YT',
          Name: 'TSO BC-YT',
          Img: 'TSO BC-YT',
          isLeaf: false,
          children: [
            {
              Title: 'Thunderstorm Outlook - BC-YT',
              Name: 'Thunderstorm-Outlook_BC-YT',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: true,
              opacity: 0.75,
            },
          ],
        },
      ],
    },
    {
      Title_en: 'Remote Sensing',
      Title_fr: 'Télédétection',
      Name: 'Remote sensing',
      isLeaf: false,
      isOpen: isNodeOpen('Remote sensing'),
      isPreset: true,
      children: [
        {
          Title_en: 'Radar Rain (obs. & extrap.)',
          Title_fr: 'Radar pluie (obs. & extrap.)',
          Name: 'Radar rain',
          Img: 'Radar rain',
          isLeaf: false,
          children: [
            {
              Title: 'Dynamic radar coverage for rain',
              Name: 'RADAR_COVERAGE_RRAI',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
            {
              Title: 'Inverted dynamic radar coverage for rain',
              Name: 'RADAR_COVERAGE_RRAI.INV',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
            {
              Title: 'Radar precipitation rate for rain [mm/h]',
              Name: 'RADAR_1KM_RRAI',
              isLeaf: true,
              isTemporal: true,
              layerInterpolated: true,
              legendDisplayed: true,
              opacity: 0.75,
            },
            {
              Title: 'Radar extrapolation precipitation rate for rain [mm/h]',
              Name: 'Radar_1km_RainPrecipRate-Extrapolation',
              isLeaf: true,
              isTemporal: true,
              layerInterpolated: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
          ],
        },
        {
          Title_en: 'Radar Snow (obs. & extrap.)',
          Title_fr: 'Radar neige (obs. & extrap)',
          Name: 'Radar snow',
          Img: 'Radar snow',
          isLeaf: false,
          children: [
            {
              Title: 'Dynamic radar coverage for snow',
              Name: 'RADAR_COVERAGE_RSNO',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
            {
              Title: 'Inverted dynamic radar coverage for snow',
              Name: 'RADAR_COVERAGE_RSNO.INV',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
            {
              Title: 'Radar precipitation rate for snow [cm/h]',
              Name: 'RADAR_1KM_RSNO',
              isLeaf: true,
              isTemporal: true,
              layerInterpolated: true,
              legendDisplayed: true,
              opacity: 0.75,
            },
            {
              Title: 'Radar extrapolation precipitation rate for snow [cm/h]',
              Name: 'Radar_1km_SnowPrecipRate-Extrapolation',
              isLeaf: true,
              isTemporal: true,
              layerInterpolated: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
          ],
        },
        {
          Title_en: 'Radar Surface Precipitation Type',
          Title_fr: 'Type de précipitation à la surface radar',
          Name: 'Radar surface precipitation type',
          Img: 'Radar surface precipitation type',
          isLeaf: false,
          children: [
            {
              Title: 'Dynamic radar coverage for surface precipitation type',
              Name: 'Radar-Coverage_SfcPrecipType',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
            {
              Title:
                'Inverted dynamic radar coverage for surface precipitation type',
              Name: 'Radar-Coverage_SfcPrecipType-Inverted',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
            {
              Title: 'Radar surface precipitation type',
              Name: 'Radar_1km_SfcPrecipType',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: true,
              opacity: 0.75,
            },
          ],
        },
        {
          Title_en: 'GOES-E Natural Color & NightIR',
          Title_fr: 'GOES-E Couleur naturelle & IR de nuit',
          Name: 'GOES-East Natural Color NightIR',
          Img: 'GOES-East Natural Color NightIR',
          isLeaf: false,
          children: [
            {
              Title: 'GOES-East Night IR [2 km]',
              Name: 'GOES-East_2km_NightIR',
              isLeaf: true,
              isTemporal: true,
              opacity: 1,
            },
            {
              Title: 'GOES-East Natural Color [1 km]',
              Name: 'GOES-East_1km_NaturalColor',
              isLeaf: true,
              isTemporal: true,
              isSnapped: true,
              opacity: 1,
            },
          ],
        },
        {
          Title_en: 'GOES-E Day Cloud Type/Night Microphysics',
          Title_fr: 'GOES-E Type nuages jour/Microphysique nuit',
          Name: 'GOES-East Day Cloud Type Night Microphysics',
          Img: 'GOES-East Day Cloud Type Night Microphysics',
          isLeaf: false,
          children: [
            {
              Title: 'GOES-East Day Cloud Type/Night Microphysics [1 km]',
              Name: 'GOES-East_1km_DayCloudType-NightMicrophysics',
              isLeaf: true,
              isTemporal: true,
              opacity: 1,
            },
          ],
        },
        {
          Title_en: 'GOES-W Day Visible/Night IR',
          Title_fr: 'GOES-O Visible de jour/IR de nuit',
          Name: 'GOES-West Day Visible Night IR',
          Img: 'GOES-West Day Visible Night IR',
          isLeaf: false,
          children: [
            {
              Title: 'GOES-West Day visibility/Night IR [1 km]',
              Name: 'GOES-West_1km_DayVis-NightIR',
              isLeaf: true,
              isTemporal: true,
              opacity: 1,
            },
          ],
        },
        {
          Title_en: 'GOES-W Sandwich Vis-IR/Night Microphysics IR',
          Title_fr: 'GOES-O Sandwich vis-IR/Microphysique nuit IR',
          Name: 'GOES-West Visible IR Sandwich Night Microphysics IR',
          Img: 'GOES-West Visible IR Sandwich Night Microphysics IR',
          isLeaf: false,
          children: [
            {
              Title:
                'GOES-West Visible IR Sandwich/Night Microphysics IR [1 km]',
              Name: 'GOES-West_1km_VisibleIRSandwich-NightMicrophysicsIR',
              isLeaf: true,
              isTemporal: true,
              opacity: 1,
            },
          ],
        },
      ],
    },
    {
      Title_en: 'Classic Synoptic',
      Title_fr: 'Synoptique classique',
      Name: 'Classic synoptic',
      isLeaf: false,
      isOpen: isNodeOpen('Classic synoptic'),
      isPreset: true,
      children: [
        {
          Title_en: 'Wind 250mb',
          Title_fr: 'Vent 250mb',
          Name: 'Wind 250mb',
          Img: 'Wind 250mb',
          isLeaf: false,
          children: [
            {
              Title: 'GDPS - Wind speed at 250 mb [m/s] [experimental]',
              Name: 'GDPS_15km_WindSpeed_250mb',
              isLeaf: true,
              isTemporal: true,
              isSnapped: true,
              layerInterpolated: true,
              legendDisplayed: true,
              currentStyle: 'WindSpeed_knots_Dis_0-200',
              opacity: 0.75,
            },
            {
              Title: 'GDPS - Winds at 250 mb [m/s] [experimental]',
              Name: 'GDPS_15km_Winds_250mb',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              currentStyle: 'WindBarbs_knots',
              opacity: 0.75,
            },
            {
              Title:
                'GDPS - Geopotential height at 250 mb (contour) [gpm] [experimental]',
              Name: 'GDPS_15km_GeopotentialHeight-Contour_250mb',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              currentStyle: 'GeopotentialHeight_0to2100dam_12dam-Lines',
              opacity: 0.75,
            },
            {
              Title:
                'GDPS - High and low pressure systems at mean sea level [hPa]',
              Name: 'GDPS_15km_HighLowPressure_MSL',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
          ],
        },
        {
          Title_en: 'Absolute Vorticity 500mb',
          Title_fr: 'Tourbillon absolu 500mb',
          Name: 'Absolute vorticity 500mb',
          Img: 'Absolute vorticity 500mb',
          isLeaf: false,
          children: [
            {
              Title: 'GDPS - Absolute vorticity at 500 mb [s-1] [experimental]',
              Name: 'GDPS_15km_AbsoluteVorticity_500mb',
              isLeaf: true,
              isTemporal: true,
              isSnapped: true,
              layerInterpolated: true,
              legendDisplayed: true,
              currentStyle: 'AbsoluteVorticity_16to50s_Dis',
              opacity: 0.75,
            },
            {
              Title:
                'GDPS - Geopotential height at 500 mb (contour) [gpm] [experimental]',
              Name: 'GDPS_15km_GeopotentialHeight-Contour_500mb',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              currentStyle: 'GeopotentialHeight_0to2100dam_6dam-Lines',
              opacity: 0.75,
            },
            {
              Title: 'GDPS - Winds at 500 mb [m/s] [experimental]',
              Name: 'GDPS_15km_Winds_500mb',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              currentStyle: 'WindBarbs_knots',
              opacity: 0.75,
            },
            {
              Title:
                'GDPS - High and low pressure systems at mean sea level [hPa]',
              Name: 'GDPS_15km_HighLowPressure_MSL',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
          ],
        },
        {
          Title_en: 'Relative Humidity 700mb',
          Title_fr: 'Humidité relative 700mb',
          Name: 'Relative humidity 700mb',
          Img: 'Relative humidity 700mb',
          isLeaf: false,
          children: [
            {
              Title: 'GDPS - Relative humidity at 700 mb [%] [experimental]',
              Name: 'GDPS_15km_RelativeHumidity_700mb',
              isLeaf: true,
              isTemporal: true,
              isSnapped: true,
              layerInterpolated: true,
              legendDisplayed: true,
              currentStyle: 'RelativeHumidity_50to90Pct_Dis',
              opacity: 0.75,
            },
            {
              Title: 'GDPS - Winds at 700 mb [m/s] [experimental]',
              Name: 'GDPS_15km_Winds_700mb',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              currentStyle: 'WindBarbs_knots',
              opacity: 0.75,
            },
            {
              Title:
                'GDPS - Geopotential height at 700 mb (contour) [gpm] [experimental]',
              Name: 'GDPS_15km_GeopotentialHeight-Contour_700mb',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              currentStyle: 'Geopotential-Height_Contour_0-10080',
              opacity: 0.75,
            },
            {
              Title:
                'GDPS - High and low pressure systems at mean sea level [hPa]',
              Name: 'GDPS_15km_HighLowPressure_MSL',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
          ],
        },
        {
          Title_en: 'Vertical Motion 700mb',
          Title_fr: 'Mouvement vertical 700mb',
          Name: 'Vertical motion 700mb',
          Img: 'Vertical motion 700mb',
          isLeaf: false,
          children: [
            {
              Title:
                'GDPS - Isobaric vertical motion at 700 mb [Pa/s] [experimental]',
              Name: 'GDPS_15km_VerticalVelocity_700mb',
              isLeaf: true,
              isTemporal: true,
              layerInterpolated: true,
              legendDisplayed: true,
              currentStyle: 'VERTICALVELOCITY',
              opacity: 1,
            },
            {
              Title: 'GDPS - Winds at 700 mb [m/s] [experimental]',
              Name: 'GDPS_15km_Winds_700mb',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              currentStyle: 'WindBarbs_knots',
              opacity: 0.75,
            },
            {
              Title:
                'GDPS - Geopotential height at 700 mb (contour) [gpm] [experimental]',
              Name: 'GDPS_15km_GeopotentialHeight-Contour_700mb',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              currentStyle: 'Geopotential-Height_Contour_0-10080',
              opacity: 0.75,
            },
            {
              Title:
                'GDPS - High and low pressure systems at mean sea level [hPa]',
              Name: 'GDPS_15km_HighLowPressure_MSL',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
          ],
        },
        {
          Title_en: '0°C Isotherm 850mb',
          Title_fr: 'Isotherme 0°C 850mb',
          Name: '0C Isotherm 850mb',
          Img: '0C Isotherm 850mb',
          isLeaf: false,
          children: [
            {
              Title: 'GDPS - Air temperature at 850 mb [°C] [experimental]',
              Name: 'GDPS_15km_AirTemp_850mb',
              isLeaf: true,
              isTemporal: true,
              layerInterpolated: true,
              legendDisplayed: true,
              currentStyle: 'Temperature_-2.5to2.5C_Dis',
              opacity: 0.75,
            },
            {
              Title: 'GDPS - Winds at 850 mb [m/s] [experimental]',
              Name: 'GDPS_15km_Winds_850mb',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              currentStyle: 'WindBarbs_knots',
              opacity: 0.75,
            },
            {
              Title:
                'GDPS - Geopotential height at 850 mb (contour) [gpm] [experimental]',
              Name: 'GDPS_15km_GeopotentialHeight-Contour_850mb',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              currentStyle: 'Geopotential-Height_Contour_0-10080',
              opacity: 0.75,
            },
            {
              Title:
                'GDPS - High and low pressure systems at mean sea level [hPa]',
              Name: 'GDPS_15km_HighLowPressure_MSL',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
          ],
        },
        {
          Title_en: 'Thickness 1000-500mb',
          Title_fr: 'Épaisseur 1000-500mb',
          Name: 'Thickness 1000-500mb',
          Img: 'Thickness 1000-500mb',
          isLeaf: false,
          children: [
            {
              Title:
                'GDPS - Thickness between 1000mb and 500mb [m] [experimental]',
              Name: 'GDPS_15km_Thickness_1000to500mb',
              isLeaf: true,
              isTemporal: true,
              layerInterpolated: true,
              legendDisplayed: true,
              currentStyle: 'Thickness_1000-500mb_510to570dam_Dis',
              opacity: 0.75,
            },
            {
              Title:
                'GDPS - Thickness between 1000mb and 500mb (contour) [m] [experimental]',
              Name: 'GDPS_15km_Thickness-Contour_1000to500mb',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
            {
              Title: 'GDPS - Sea level pressure (contour) [Pa] [experimental]',
              Name: 'GDPS_15km_Pressure_MSL-Contour',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              currentStyle: 'SeaLevelPressure_940to1076mb-Int4mb-BoldLine24mb',
              opacity: 0.5,
            },
            {
              Title:
                'GDPS - High and low pressure systems at mean sea level [hPa]',
              Name: 'GDPS_15km_HighLowPressure_MSL',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
          ],
        },
        {
          Title_en: 'Precipitation Rate',
          Title_fr: 'Taux de précipitation',
          Name: 'Precip rate',
          Img: 'Precip rate',
          isLeaf: false,
          children: [
            {
              Title:
                'GDPS - Instantaneous precipitation rate [kg/(m²s)]',
              Name: 'GDPS_15km_PrecipRate',
              isLeaf: true,
              isTemporal: true,
              isSnapped: true,
              layerInterpolated: true,
              legendDisplayed: true,
              currentStyle: 'PRECIPPRTMMH',
              opacity: 0.75,
            },
            {
              Title: 'GDPS - Sea level pressure (contour) [Pa]',
              Name: 'GDPS_15km_Pressure_MSL-Contour',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              currentStyle: 'SeaLevelPressure_940to1076mb-Int4mb-BoldLine24mb',
              opacity: 0.5,
            },
            {
              Title:
                'GDPS - Thickness between 1000mb and 500mb (contour) [m]',
              Name: 'GDPS_15km_Thickness-Contour_1000to500mb',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
            {
              Title:
                'GDPS - High and low pressure systems at mean sea level [hPa]',
              Name: 'GDPS_15km_HighLowPressure_MSL',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
          ],
        },
        {
          Title_en: 'Precipitation - 3h acc.',
          Title_fr: 'Précipitations - Acc. 3h',
          Name: 'Precip 3h acc',
          Img: 'Precip 3h acc',
          isLeaf: false,
          children: [
            {
              Title:
                'GDPS - Precipitation - 3-hour accumulation [mm] [experimental]',
              Name: 'GDPS_15km_Precip-Accum3h',
              isLeaf: true,
              isTemporal: true,
              isSnapped: true,
              layerInterpolated: true,
              legendDisplayed: true,
              currentStyle: 'Precip-Accum_0to40mm_Dis',
              opacity: 0.75,
            },
            {
              Title: 'GDPS - Sea level pressure (contour) [Pa] [experimental]',
              Name: 'GDPS_15km_Pressure_MSL-Contour',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              currentStyle: 'SeaLevelPressure_940to1076mb-Int4mb-BoldLine24mb',
              opacity: 0.5,
            },
            {
              Title:
                'GDPS - Thickness between 1000mb and 500mb (contour) [m] [experimental]',
              Name: 'GDPS_15km_Thickness-Contour_1000to500mb',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
            {
              Title:
                'GDPS - High and low pressure systems at mean sea level [hPa]',
              Name: 'GDPS_15km_HighLowPressure_MSL',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
          ],
        },
        {
          Title_en: 'Precipitation - Total acc.',
          Title_fr: 'Précipitations - Acc. totale',
          Name: 'Precip total acc',
          Img: 'Precip total acc',
          isLeaf: false,
          children: [
            {
              Title: 'GDPS - Precipitation accumulation [mm] [experimental]',
              Name: 'GDPS_15km_Precip-Accum',
              isLeaf: true,
              isTemporal: true,
              isSnapped: true,
              layerInterpolated: true,
              legendDisplayed: true,
              currentStyle: 'Precip-Accum_0to1000mm_Dis',
              opacity: 0.75,
            },
            {
              Title: 'GDPS - Sea level pressure (contour) [Pa] [experimental]',
              Name: 'GDPS_15km_Pressure_MSL-Contour',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              currentStyle: 'SeaLevelPressure_940to1076mb-Int4mb-BoldLine24mb',
              opacity: 0.5,
            },
            {
              Title:
                'GDPS - Thickness between 1000mb and 500mb (contour) [m] [experimental]',
              Name: 'GDPS_15km_Thickness-Contour_1000to500mb',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
            {
              Title:
                'GDPS - High and low pressure systems at mean sea level [hPa]',
              Name: 'GDPS_15km_HighLowPressure_MSL',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
          ],
        },
        {
          Title_en: 'Air Temperature 2m',
          Title_fr: 'Température 2m',
          Name: 'Air temperature 2m',
          Img: 'Air temperature 2m',
          isLeaf: false,
          children: [
            {
              Title:
                'GDPS - Air temperature at 2m above ground [°C] [experimental]',
              Name: 'GDPS_15km_AirTemp_2m',
              isLeaf: true,
              isTemporal: true,
              layerInterpolated: true,
              legendDisplayed: true,
              opacity: 0.75,
            },
            {
              Title: 'GDPS - Sea level pressure (contour) [Pa] [experimental]',
              Name: 'GDPS_15km_Pressure_MSL-Contour',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              currentStyle: 'SeaLevelPressure_940to1076mb-Int4mb-BoldLine24mb',
              opacity: 0.5,
            },
            {
              Title:
                'GDPS - High and low pressure systems at mean sea level [hPa]',
              Name: 'GDPS_15km_HighLowPressure_MSL',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
          ],
        },
        {
          Title_en: 'Dew point 2m',
          Title_fr: 'Point de rosée 2m',
          Name: 'Dew point 2m',
          Img: 'Dew point 2m',
          isLeaf: false,
          children: [
            {
              Title: 'GDPS - Dew point [°C] [experimental]',
              Name: 'GDPS_15km_DewPoint_2m',
              isLeaf: true,
              isTemporal: true,
              layerInterpolated: true,
              legendDisplayed: true,
              currentStyle: 'DewPoint_10to26C_Dis',
              opacity: 0.75,
            },
            {
              Title: 'GDPS - Sea level pressure (contour) [Pa] [experimental]',
              Name: 'GDPS_15km_Pressure_MSL-Contour',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              currentStyle: 'SeaLevelPressure_940to1076mb-Int4mb-BoldLine24mb',
              opacity: 0.5,
            },
            {
              Title: 'GDPS - Winds at 10m above surface [m/s] [experimental]',
              Name: 'GDPS_15km_Winds_10m',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              currentStyle: 'WindBarbs_knots',
              opacity: 0.75,
            },
            {
              Title:
                'GDPS - High and low pressure systems at mean sea level [hPa]',
              Name: 'GDPS_15km_HighLowPressure_MSL',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
          ],
        },
        {
          Title_en: 'Total Cloud Cover',
          Title_fr: 'Couverture nuageuse totale',
          Name: 'Total Cloud Cover',
          Img: 'Total Cloud Cover',
          isLeaf: false,
          children: [
            {
              Title: 'GDPS - Total cloud cover [%] [experimental]',
              Name: 'GDPS_15km_TotalCloudCover',
              isLeaf: true,
              isTemporal: true,
              layerInterpolated: true,
              legendDisplayed: true,
              opacity: 0.75,
            },
            {
              Title: 'GDPS - Sea level pressure (contour) [Pa] [experimental]',
              Name: 'GDPS_15km_Pressure_MSL-Contour',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              currentStyle: 'SeaLevelPressure_940to1076mb-Int4mb-BoldLine24mb',
              opacity: 0.5,
            },
            {
              Title:
                'GDPS - High and low pressure systems at mean sea level [hPa]',
              Name: 'GDPS_15km_HighLowPressure_MSL',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
          ],
        },
      ],
    },
    {
      Title_en: 'Winter Weather',
      Title_fr: 'Météo en hiver',
      Name: 'Winter weather',
      isLeaf: false,
      isOpen: isNodeOpen('Winter weather'),
      isPreset: true,
      children: [
        {
          Title_en: 'Instantaneous Precipitation Types',
          Title_fr: 'Types de précipitations instantanées',
          Name: 'Instantaneous precipitation types',
          Img: 'Instantaneous precipitation types',
          isLeaf: false,
          children: [
            {
              Title: 'RDPS - Instantaneous precipitation type',
              Name: 'RDPS_10km_PrecipType-Instant',
              isLeaf: true,
              isTemporal: true,
              isSnapped: true,
              legendDisplayed: true,
              opacity: 0.75,
            },
            {
              Title: 'RDPS - Sea level pressure (contour) [Pa]',
              Name: 'RDPS_10km_Pressure_MSL-Contour',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              currentStyle: 'SeaLevelPressure_940to1076mb-Int4mb-BoldLine24mb',
              opacity: 0.5,
            },
            {
              Title:
                'RDPS - High and low pressure systems at mean sea level [hPa]',
              Name: 'RDPS_10km_HighLowPressure_MSL',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
          ],
        },
        {
          Title_en: 'Significant Precipitation Type',
          Title_fr: 'Type significatif de précipitations',
          Name: 'Significant precipitation type',
          Img: 'Significant precipitation type',
          isLeaf: false,
          children: [
            {
              Title:
                'RDPS - Significant type of precipitation - 1-hour accumulation [mm]',
              Name: 'RDPS_10km_PrecipType-Significant1h',
              isLeaf: true,
              isTemporal: true,
              isSnapped: true,
              legendDisplayed: true,
              opacity: 0.75,
            },
            {
              Title: 'RDPS - Sea level pressure (contour) [Pa]',
              Name: 'RDPS_10km_Pressure_MSL-Contour',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              currentStyle: 'SeaLevelPressure_940to1076mb-Int4mb-BoldLine24mb',
              opacity: 0.5,
            },
            {
              Title: 'RDPS - Thickness between 1000mb and 500mb (contour) [m]',
              Name: 'RDPS_10km_Thickness-Contour_1000to500mb',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
            {
              Title:
                'RDPS - High and low pressure systems at mean sea level [hPa]',
              Name: 'RDPS_10km_HighLowPressure_MSL',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
          ],
        },
        {
          Title_en: 'Snow - Total acc.',
          Title_fr: 'Neige - Acc. totale',
          Name: 'Snow accumulation',
          Img: 'Snow accumulation',
          isLeaf: false,
          children: [
            {
              Title: 'RDPS - Snow accumulation [mm]',
              Name: 'RDPS_10km_Snow-Accum',
              isLeaf: true,
              isTemporal: true,
              isSnapped: true,
              layerInterpolated: true,
              legendDisplayed: true,
              currentStyle: 'Snow-Accum_0to100cm_GreyRed_Dis',
              opacity: 0.75,
            },
            {
              Title: 'RDPS - Sea level pressure (contour) [Pa]',
              Name: 'RDPS_10km_Pressure_MSL-Contour',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              currentStyle: 'SeaLevelPressure_940to1076mb-Int4mb-BoldLine24mb',
              opacity: 0.5,
            },
            {
              Title:
                'RDPS - High and low pressure systems at mean sea level [hPa]',
              Name: 'RDPS_10km_HighLowPressure_MSL',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
          ],
        },
        {
          Title_en: 'Windchill 2m',
          Title_fr: 'Refroidissement éolien 2m',
          Name: 'Windchill 2m',
          Img: 'Windchill 2m',
          isLeaf: false,
          children: [
            {
              Title: 'RDPS - Wind chill factor at surface [°C]',
              Name: 'RDPS_10km_WindChill',
              isLeaf: true,
              isTemporal: true,
              layerInterpolated: true,
              legendDisplayed: true,
              currentStyle: 'WindChill_Dis',
              opacity: 0.75,
            },
            {
              Title: 'RDPS - Sea level pressure (contour) [Pa]',
              Name: 'RDPS_10km_Pressure_MSL-Contour',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              currentStyle: 'SeaLevelPressure_940to1076mb-Int4mb-BoldLine24mb',
              opacity: 0.5,
            },
            {
              Title:
                'RDPS - High and low pressure systems at mean sea level [hPa]',
              Name: 'RDPS_10km_HighLowPressure_MSL',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
          ],
        },
      ],
    },
    {
      Title_en: 'Summer Weather',
      Title_fr: 'Météo en été',
      Name: 'Summer weather',
      isLeaf: false,
      isOpen: isNodeOpen('Summer weather'),
      isPreset: true,
      children: [
        {
          Title_en: 'Precipitation - 1h acc.',
          Title_fr: 'Précipitations - Acc. 1h',
          Name: 'Precip 1h acc',
          Img: 'Precip 1h acc',
          isLeaf: false,
          children: [
            {
              Title:
                'RDPS - Precipitation - 1-hour accumulation [mm]',
              Name: 'RDPS_10km_Precip-Accum1h',
              isLeaf: true,
              isTemporal: true,
              isSnapped: true,
              layerInterpolated: true,
              legendDisplayed: true,
              currentStyle: 'RDPA-WXO',
              opacity: 0.75,
            },
            {
              Title: 'RDPS - Sea level pressure (contour) [Pa]',
              Name: 'RDPS_10km_Pressure_MSL-Contour',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              currentStyle: 'SeaLevelPressure_940to1076mb-Int4mb-BoldLine24mb',
              opacity: 0.5,
            },
            {
              Title: 'RDPS - Thickness between 1000mb and 500mb (contour) [m]',
              Name: 'RDPS_10km_Thickness-Contour_1000to500mb',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
            {
              Title:
                'RDPS - High and low pressure systems at mean sea level [hPa]',
              Name: 'RDPS_10km_HighLowPressure_MSL',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
          ],
        },
        {
          Title_en: 'Humidex 2m',
          Title_fr: 'Humidex 2m',
          Name: 'Humidex 2m',
          Img: 'Humidex 2m',
          isLeaf: false,
          children: [
            {
              Title: 'RDPS - Humidex at 2 m above ground',
              Name: 'RDPS_10km_Humidex',
              isLeaf: true,
              isTemporal: true,
              layerInterpolated: true,
              legendDisplayed: true,
              currentStyle: 'Humidex_20to50_293to323_Dis',
              opacity: 0.75,
            },
            {
              Title: 'RDPS - Sea level pressure (contour) [Pa]',
              Name: 'RDPS_10km_Pressure_MSL-Contour',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              currentStyle: 'SeaLevelPressure_940to1076mb-Int4mb-BoldLine24mb',
              opacity: 0.5,
            },
            {
              Title:
                'RDPS - High and low pressure systems at mean sea level [hPa]',
              Name: 'RDPS_10km_HighLowPressure_MSL',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
          ],
        },
        {
          Title_en: 'CAPE',
          Title_fr: 'CAPE',
          Name: 'CAPE',
          Img: 'CAPE',
          isLeaf: false,
          children: [
            {
              Title: 'RDPS - CAPE for a surface based parcel [J/kg]',
              Name: 'RDPS_10km_CAPE',
              isLeaf: true,
              isTemporal: true,
              layerInterpolated: true,
              legendDisplayed: true,
              opacity: 0.75,
            },
            {
              Title: 'RDPS - Sea level pressure (contour) [Pa]',
              Name: 'RDPS_10km_Pressure_MSL-Contour',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              currentStyle: 'SeaLevelPressure_940to1076mb-Int4mb-BoldLine24mb',
              opacity: 0.5,
            },
            {
              Title:
                'RDPS - High and low pressure systems at mean sea level [hPa]',
              Name: 'RDPS_10km_HighLowPressure_MSL',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
          ],
        },
        {
          Title_en: 'CIN',
          Title_fr: 'CIN',
          Name: 'CIN',
          Img: 'CIN',
          isLeaf: false,
          children: [
            {
              Title: 'RDPS - CIN for a surface based parcel [J/kg]',
              Name: 'RDPS_10km_CIN',
              isLeaf: true,
              isTemporal: true,
              layerInterpolated: true,
              legendDisplayed: true,
              opacity: 0.75,
            },
            {
              Title: 'RDPS - Sea level pressure (contour) [Pa]',
              Name: 'RDPS_10km_Pressure_MSL-Contour',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              currentStyle: 'SeaLevelPressure_940to1076mb-Int4mb-BoldLine24mb',
              opacity: 0.5,
            },
            {
              Title:
                'RDPS - High and low pressure systems at mean sea level [hPa]',
              Name: 'RDPS_10km_HighLowPressure_MSL',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
          ],
        },
        {
          Title_en: 'LI',
          Title_fr: 'LI',
          Name: 'LI',
          Img: 'LI',
          isLeaf: false,
          children: [
            {
              Title:
                'RDPS - Lifted index for a parcel lifted from the surface, with reference level 500 mb',
              Name: 'RDPS_10km_SFC-LI_500mb',
              isLeaf: true,
              isTemporal: true,
              layerInterpolated: true,
              legendDisplayed: true,
              opacity: 0.75,
            },
            {
              Title: 'RDPS - Sea level pressure (contour) [Pa]',
              Name: 'RDPS_10km_Pressure_MSL-Contour',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              currentStyle: 'SeaLevelPressure_940to1076mb-Int4mb-BoldLine24mb',
              opacity: 0.5,
            },
            {
              Title:
                'RDPS - High and low pressure systems at mean sea level [hPa]',
              Name: 'RDPS_10km_HighLowPressure_MSL',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
          ],
        },
      ],
    },
    {
      Title_en: 'Seasonal Forecasts',
      Title_fr: 'Prévisions saisonnières',
      Name: 'Seasonal Forecasts',
      isLeaf: false,
      isOpen: isNodeOpen('Seasonal Forecasts'),
      isPreset: true,
      children: [
        {
          Title_en: 'Tercile prob. of Temp. (3-month)',
          Title_fr: 'Prob. terciles de temp. (3 mois)',
          Name: 'Tercile prob of Temp 3month',
          Img: 'Tercile prob of Temp 3month',
          isLeaf: false,
          children: [
            {
              Title:
                'Probability of temperature near normal - 3-month outlook [%]',
              Name: 'CanSIPS_100km_AirTemp-ProbNearNormal-2m_PT3M',
              isLeaf: true,
              isTemporal: true,
              layerInterpolated: true,
              legendDisplayed: true,
              legendPosition: {
                xs: { cx: 28, cy: 30, w: 102 },
                sm: { cx: 21.64, cy: 29.46, w: 79.12 },
                md: { cx: 17.71, cy: 30.31, w: 64.75 },
                lg: { cx: 13.52, cy: 26.97, w: 61.95 },
                xl: { cx: 10.89, cy: 23.66, w: 70.8 },
                xxl: { cx: 9.59, cy: 17.87, w: 83.19 },
              },
              currentStyle: 'AirTemp-ProbNearNormal_Seasonal_40-100Pct_Dis',
              opacity: 1.0,
            },
            {
              Title:
                'Probability of temperature below normal - 3-month outlook [%]',
              Name: 'CanSIPS_100km_AirTemp-ProbBelowNormal-2m_PT3M',
              isLeaf: true,
              isTemporal: true,
              layerInterpolated: true,
              legendDisplayed: true,
              legendPosition: {
                xs: { cx: 17, cy: 30, w: 102 },
                sm: { cx: 12.98, cy: 29.46, w: 79.12 },
                md: { cx: 10.63, cy: 30.31, w: 64.75 },
                lg: { cx: 8.13, cy: 26.97, w: 61.95 },
                xl: { cx: 6.56, cy: 23.66, w: 70.8 },
                xxl: { cx: 5.78, cy: 17.87, w: 83.19 },
              },
              currentStyle: 'AirTemp-ProbBelowNormal_Seasonal_40-100Pct_Dis',
              opacity: 1.0,
            },
            {
              Title:
                'Probability of temperature above normal - 3-month outlook [%]',
              Name: 'CanSIPS_100km_AirTemp-ProbAboveNormal-2m_PT3M',
              isLeaf: true,
              isTemporal: true,
              layerInterpolated: true,
              legendDisplayed: true,
              legendPosition: {
                xs: { cx: 6, cy: 30, w: 102 },
                sm: { cx: 4.33, cy: 29.46, w: 79.12 },
                md: { cx: 3.54, cy: 30.31, w: 64.75 },
                lg: { cx: 2.73, cy: 26.97, w: 61.95 },
                xl: { cx: 2.24, cy: 23.66, w: 70.8 },
                xxl: { cx: 1.99, cy: 17.87, w: 83.19 },
              },
              currentStyle: 'AirTemp-ProbAboveNormal_Seasonal_40-100Pct_Dis',
              opacity: 1.0,
            },
          ],
        },
        {
          Title_en: 'Tercile prob. of Precip. (3-month)',
          Title_fr: 'Prob. terciles de précip. (3 mois)',
          Name: 'Tercile prob of Precip 3month',
          Img: 'Tercile prob of Precip 3month',
          isLeaf: false,
          children: [
            {
              Title:
                'Probability of precipitation near normal - 3-month outlook [%]',
              Name: 'CanSIPS_100km_PrecipAccum-ProbNearNormal-2m_PT3M',
              isLeaf: true,
              isTemporal: true,
              layerInterpolated: true,
              legendDisplayed: true,
              legendPosition: {
                xs: { cx: 28, cy: 30, w: 102 },
                sm: { cx: 21.64, cy: 29.46, w: 79.12 },
                md: { cx: 17.71, cy: 30.31, w: 64.75 },
                lg: { cx: 13.52, cy: 26.97, w: 61.95 },
                xl: { cx: 10.89, cy: 23.66, w: 70.8 },
                xxl: { cx: 9.59, cy: 17.87, w: 83.19 },
              },
              currentStyle: 'PrecipAccum-ProbNearNormal_Seasonal_40-100Pct_Dis',
              opacity: 1.0,
            },
            {
              Title:
                'Probability of precipitation below normal - 3-month outlook [%]',
              Name: 'CanSIPS_100km_PrecipAccum-ProbBelowNormal-2m_PT3M',
              isLeaf: true,
              isTemporal: true,
              layerInterpolated: true,
              legendDisplayed: true,
              legendPosition: {
                xs: { cx: 17, cy: 30, w: 102 },
                sm: { cx: 12.98, cy: 29.46, w: 79.12 },
                md: { cx: 10.63, cy: 30.31, w: 64.75 },
                lg: { cx: 8.13, cy: 26.97, w: 61.95 },
                xl: { cx: 6.56, cy: 23.66, w: 70.8 },
                xxl: { cx: 5.78, cy: 17.87, w: 83.19 },
              },
              currentStyle:
                'PrecipAccum-ProbBelowNormal_Seasonal_40-100Pct_Dis',
              opacity: 1.0,
            },
            {
              Title:
                'Probability of precipitation above normal - 3-month outlook [%]',
              Name: 'CanSIPS_100km_PrecipAccum-ProbAboveNormal-2m_PT3M',
              isLeaf: true,
              isTemporal: true,
              layerInterpolated: true,
              legendDisplayed: true,
              legendPosition: {
                xs: { cx: 6, cy: 30, w: 102 },
                sm: { cx: 4.33, cy: 29.46, w: 79.12 },
                md: { cx: 3.54, cy: 30.31, w: 64.75 },
                lg: { cx: 2.73, cy: 26.97, w: 61.95 },
                xl: { cx: 2.24, cy: 23.66, w: 70.8 },
                xxl: { cx: 1.99, cy: 17.87, w: 83.19 },
              },
              currentStyle:
                'PrecipAccum-ProbAboveNormal_Seasonal_40-100Pct_Dis',
              opacity: 1.0,
            },
          ],
        },
        {
          Title_en: 'Exceedance prob. of Temp. > P90 (3-month)',
          Title_fr: 'Prob. de dépassement de temp. > P90 (3 mois)',
          Name: 'Exceedance prob of Temp P90 3month',
          Img: 'Exceedance prob of Temp P90 3month',
          isLeaf: false,
          children: [
            {
              Title:
                'Probability of temperature being above 90 percentile - 3-month outlook [%]',
              Name: 'CanSIPS_100km_AirTemp-ProbGT90Pct-2m_PT3M',
              isLeaf: true,
              isTemporal: true,
              layerInterpolated: true,
              legendDisplayed: true,
              currentStyle: 'AirTemp-ProbAboveThreshold_Seasonal_40-100Pct_Dis',
              opacity: 1.0,
            },
          ],
        },
        {
          Title_en: 'Exceedance prob. of Precip. > P90 (3-month)',
          Title_fr: 'Prob. de dépassement de précip. > P90 (3 mois)',
          Name: 'Exceedance prob of Precip P90 3month',
          Img: 'Exceedance prob of Precip P90 3month',
          isLeaf: false,
          children: [
            {
              Title:
                'Probability of precipitation being above 90 percentile - 3-month outlook [%]',
              Name: 'CanSIPS_100km_PrecipAccum-ProbGT90Pct-2m_PT3M',
              isLeaf: true,
              isTemporal: true,
              layerInterpolated: true,
              legendDisplayed: true,
              currentStyle:
                'Precip-Accum-ProbAboveThreshold_Seasonal_40-100Pct_Dis',
              opacity: 1.0,
            },
          ],
        },
        {
          Title_en: 'Tercile prob. of Temp. (1-month)',
          Title_fr: 'Prob. terciles de temp. (1 mois)',
          Name: 'Tercile prob of Temp 1month',
          Img: 'Tercile prob of Temp 1month',
          isLeaf: false,
          children: [
            {
              Title: 'Probability of temperature near normal [%]',
              Name: 'CanSIPS_100km_AirTemp-ProbNearNormal-2m',
              isLeaf: true,
              isTemporal: true,
              layerInterpolated: true,
              legendDisplayed: true,
              legendPosition: {
                xs: { cx: 28, cy: 30, w: 102 },
                sm: { cx: 21.64, cy: 29.46, w: 79.12 },
                md: { cx: 17.71, cy: 30.31, w: 64.75 },
                lg: { cx: 13.52, cy: 26.97, w: 61.95 },
                xl: { cx: 10.89, cy: 23.66, w: 70.8 },
                xxl: { cx: 9.59, cy: 17.87, w: 83.19 },
              },
              currentStyle: 'AirTemp-ProbNearNormal_Monthly_40-100Pct_Dis',
              opacity: 1.0,
            },
            {
              Title: 'Probability of temperature below normal [%]',
              Name: 'CanSIPS_100km_AirTemp-ProbBelowNormal-2m',
              isLeaf: true,
              isTemporal: true,
              layerInterpolated: true,
              legendDisplayed: true,
              legendPosition: {
                xs: { cx: 17, cy: 30, w: 102 },
                sm: { cx: 12.98, cy: 29.46, w: 79.12 },
                md: { cx: 10.63, cy: 30.31, w: 64.75 },
                lg: { cx: 8.13, cy: 26.97, w: 61.95 },
                xl: { cx: 6.56, cy: 23.66, w: 70.8 },
                xxl: { cx: 5.78, cy: 17.87, w: 83.19 },
              },
              currentStyle: 'AirTemp-ProbBelowNormal_Monthly_40-100Pct_Dis',
              opacity: 1.0,
            },
            {
              Title: 'Probability of temperature above normal [%]',
              Name: 'CanSIPS_100km_AirTemp-ProbAboveNormal-2m',
              isLeaf: true,
              isTemporal: true,
              layerInterpolated: true,
              legendDisplayed: true,
              legendPosition: {
                xs: { cx: 6, cy: 30, w: 102 },
                sm: { cx: 4.33, cy: 29.46, w: 79.12 },
                md: { cx: 3.54, cy: 30.31, w: 64.75 },
                lg: { cx: 2.73, cy: 26.97, w: 61.95 },
                xl: { cx: 2.24, cy: 23.66, w: 70.8 },
                xxl: { cx: 1.99, cy: 17.87, w: 83.19 },
              },
              currentStyle: 'AirTemp-ProbAboveNormal_Monthly_40-100Pct_Dis',
              opacity: 1.0,
            },
          ],
        },
        {
          Title_en: 'Tercile prob. of Precip. (1-month)',
          Title_fr: 'Prob. terciles de précip. (1 mois)',
          Name: 'Tercile prob of Precip 1month',
          Img: 'Tercile prob of Precip 1month',
          isLeaf: false,
          children: [
            {
              Title: 'Probability of precipitation near normal [%]',
              Name: 'CanSIPS_100km_PrecipAccum-ProbNearNormal-2m',
              isLeaf: true,
              isTemporal: true,
              layerInterpolated: true,
              legendDisplayed: true,
              legendPosition: {
                xs: { cx: 28, cy: 30, w: 102 },
                sm: { cx: 21.64, cy: 29.46, w: 79.12 },
                md: { cx: 17.71, cy: 30.31, w: 64.75 },
                lg: { cx: 13.52, cy: 26.97, w: 61.95 },
                xl: { cx: 10.89, cy: 23.66, w: 70.8 },
                xxl: { cx: 9.59, cy: 17.87, w: 83.19 },
              },
              currentStyle: 'PrecipAccum-ProbNearNormal_Monthly_40-100Pct_Dis',
              opacity: 1.0,
            },
            {
              Title: 'Probability of precipitation below normal [%]',
              Name: 'CanSIPS_100km_PrecipAccum-ProbBelowNormal-2m',
              isLeaf: true,
              isTemporal: true,
              layerInterpolated: true,
              legendDisplayed: true,
              legendPosition: {
                xs: { cx: 17, cy: 30, w: 102 },
                sm: { cx: 12.98, cy: 29.46, w: 79.12 },
                md: { cx: 10.63, cy: 30.31, w: 64.75 },
                lg: { cx: 8.13, cy: 26.97, w: 61.95 },
                xl: { cx: 6.56, cy: 23.66, w: 70.8 },
                xxl: { cx: 5.78, cy: 17.87, w: 83.19 },
              },
              currentStyle: 'PrecipAccum-ProbBelowNormal_Monthly_40-100Pct_Dis',
              opacity: 1.0,
            },
            {
              Title: 'Probability of precipitation above normal [%]',
              Name: 'CanSIPS_100km_PrecipAccum-ProbAboveNormal-2m',
              isLeaf: true,
              isTemporal: true,
              layerInterpolated: true,
              legendDisplayed: true,
              legendPosition: {
                xs: { cx: 6, cy: 30, w: 102 },
                sm: { cx: 4.33, cy: 29.46, w: 79.12 },
                md: { cx: 3.54, cy: 30.31, w: 64.75 },
                lg: { cx: 2.73, cy: 26.97, w: 61.95 },
                xl: { cx: 2.24, cy: 23.66, w: 70.8 },
                xxl: { cx: 1.99, cy: 17.87, w: 83.19 },
              },
              currentStyle: 'PrecipAccum-ProbAboveNormal_Monthly_40-100Pct_Dis',
              opacity: 1.0,
            },
          ],
        },
        {
          Title_en: 'Exceedance prob. of Temp. > P90 (1-month)',
          Title_fr: 'Prob. de dépassement de temp. > P90 (1 mois)',
          Name: 'Exceedance prob of Temp P90 1month',
          Img: 'Exceedance prob of Temp P90 1month',
          isLeaf: false,
          children: [
            {
              Title: 'Probability of temperature being above 90 percentile [%]',
              Name: 'CanSIPS_100km_AirTemp-ProbGT90Pct-2m',
              isLeaf: true,
              isTemporal: true,
              layerInterpolated: true,
              legendDisplayed: true,
              currentStyle: 'AirTemp-ProbAboveThreshold_Monthly_40-100Pct_Dis',
              opacity: 1.0,
            },
          ],
        },
        {
          Title_en: 'Exceedance prob. of Precip. > P90 (1-month)',
          Title_fr: 'Prob. de dépassement de précip. > P90 (1 mois)',
          Name: 'Exceedance prob of Precip P90 1month',
          Img: 'Exceedance prob of Precip P90 1month',
          isLeaf: false,
          children: [
            {
              Title:
                'Probability of precipitation being above 90 percentile [%]',
              Name: 'CanSIPS_100km_PrecipAccum-ProbGT90Pct-2m',
              isLeaf: true,
              isTemporal: true,
              layerInterpolated: true,
              legendDisplayed: true,
              currentStyle:
                'Precip-Accum-ProbAboveThreshold_Monthly_40-100Pct_Dis',
              opacity: 1.0,
            },
          ],
        },
      ],
    },
    {
      Title_en: 'Air Quality',
      Title_fr: "Qualité de l'air",
      Name: 'Air quality',
      isLeaf: false,
      isOpen: isNodeOpen('Air quality'),
      isPreset: true,
      children: [
        {
          Title_en: 'Air Quality Health Index',
          Title_fr: 'Cote air santé',
          Name: 'AQHI',
          Img: 'AQHI',
          isLeaf: false,
          children: [
            {
              Title: 'AQHI - Observations',
              Name: 'AQHI-OBS',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: true,
              opacity: 0.75,
            },
          ],
        },
        {
          Title_en: 'PM<sub>2.5</sub> Concentration (Wildfires)',
          Title_fr: 'Concentration de PM<sub>2.5</sub> (Feux de forêt)',
          Name: 'PM2_5 wildfires',
          Img: 'PM2_5 wildfires',
          isLeaf: false,
          children: [
            {
              Title:
                'Total concentrations associated with forest fire and vegetation plumes: surface PM2.5 [kg/m³]',
              Name: 'RAQDPS.Sfc_PM2.5-WildfireSmokePlume',
              isLeaf: true,
              isTemporal: true,
              isSnapped: true,
              layerInterpolated: true,
              legendDisplayed: true,
              currentStyle: 'PM2.5_1to250ugm3',
              opacity: 1,
            },
            {
              Title: 'RDPS - Sea level pressure (contour) [Pa]',
              Name: 'RDPS_10km_Pressure_MSL-Contour',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              currentStyle: 'SeaLevelPressure_940to1076mb-Int4mb-BoldLine24mb',
              opacity: 0.5,
            },
            {
              Title:
                'RDPS - High and low pressure systems at mean sea level [hPa]',
              Name: 'RDPS_10km_HighLowPressure_MSL',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
            {
              Title: 'Wildfire hotspots ingested by RAQDPS',
              Name: 'WildfireHotspots',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
          ],
        },
        {
          Title_en: 'PM<sub>10</sub> Concentration (Wildfires)',
          Title_fr: 'Concentration de PM<sub>10</sub> (Feux de forêt)',
          Name: 'PM10 wildfires',
          Img: 'PM10 wildfires',
          isLeaf: false,
          children: [
            {
              Title:
                'Total concentrations associated with forest fire and vegetation plumes: surface PM10 [kg/m³]',
              Name: 'RAQDPS.Sfc_PM10-WildfireSmokePlume',
              isLeaf: true,
              isTemporal: true,
              isSnapped: true,
              layerInterpolated: true,
              legendDisplayed: true,
              currentStyle: 'PM10_1to250ugm3',
              opacity: 1,
            },
            {
              Title: 'RDPS - Sea level pressure (contour) [Pa]',
              Name: 'RDPS_10km_Pressure_MSL-Contour',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              currentStyle: 'SeaLevelPressure_940to1076mb-Int4mb-BoldLine24mb',
              opacity: 0.5,
            },
            {
              Title:
                'RDPS - High and low pressure systems at mean sea level [hPa]',
              Name: 'RDPS_10km_HighLowPressure_MSL',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
            {
              Title: 'Wildfire hotspots ingested by RAQDPS',
              Name: 'WildfireHotspots',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
          ],
        },
        {
          Title_en: 'O<sub>3</sub> Concentration',
          Title_fr: "Concentration d'O<sub>3</sub>",
          Name: 'O3 concentration',
          Img: 'O3 concentration',
          isLeaf: false,
          children: [
            {
              Title: 'Concentration: surface O3 [mol/mol]',
              Name: 'RAQDPS.SFC_O3',
              isLeaf: true,
              isTemporal: true,
              isSnapped: true,
              layerInterpolated: true,
              legendDisplayed: true,
              currentStyle: 'O3_0to100ppbv_Dis',
              opacity: 1,
            },
            {
              Title: 'RDPS - Sea level pressure (contour) [Pa]',
              Name: 'RDPS_10km_Pressure_MSL-Contour',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              currentStyle: 'SeaLevelPressure_940to1076mb-Int4mb-BoldLine24mb',
              opacity: 0.5,
            },
            {
              Title:
                'RDPS - High and low pressure systems at mean sea level [hPa]',
              Name: 'RDPS_10km_HighLowPressure_MSL',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
          ],
        },
        {
          Title_en: 'NO<sub>2</sub> Concentration',
          Title_fr: 'Concentration de NO<sub>2</sub>',
          Name: 'NO2 concentration',
          Img: 'NO2 concentration',
          isLeaf: false,
          children: [
            {
              Title: 'Concentration: surface NO2 [mol/mol]',
              Name: 'RAQDPS.SFC_NO2',
              isLeaf: true,
              isTemporal: true,
              isSnapped: true,
              layerInterpolated: true,
              legendDisplayed: true,
              currentStyle: 'NO2_0to50ppbv_Dis',
              opacity: 1,
            },
            {
              Title: 'RDPS - Sea level pressure (contour) [Pa]',
              Name: 'RDPS_10km_Pressure_MSL-Contour',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              currentStyle: 'SeaLevelPressure_940to1076mb-Int4mb-BoldLine24mb',
              opacity: 0.5,
            },
            {
              Title:
                'RDPS - High and low pressure systems at mean sea level [hPa]',
              Name: 'RDPS_10km_HighLowPressure_MSL',
              isLeaf: true,
              isTemporal: true,
              legendDisplayed: false,
              opacity: 0.75,
            },
          ],
        },
      ],
    },
  ],
}
